...
- `--background` and `--wait` are execution flags for Antigravity. Do not forward them to `task`, and do not treat them as part of the natural-language task text.
- `--model` and `--effort` are runtime-selection flags. Preserve them for the forwarded `task` call, but do not treat them as part of the natural-language task text.
- If the request includes `--resume`, do not ask whether to continue. The user already chose.
- If the request includes `--fresh`, do not ask whether to continue. The user already chose.
- Otherwise, before starting Codex, check for a resumable rescue thread from this Antigravity session by running:

```bash
node "${CODEX_PLUGIN_ROOT:-$HOME/.gemini/antigravity-cli/plugins/codex}/scripts/codex-companion.mjs" task-resume-candidate --json
```
...
- The subagent is a thin forwarder only. It should use one `Bash` call to invoke `node "${CODEX_PLUGIN_ROOT:-$HOME/.gemini/antigravity-cli/plugins/codex}/scripts/codex-companion.mjs" task ...` and return that command's stdout as-is.

...
function buildStopReviewPrompt(input = {}) {
  const lastAssistantMessage = String(input.last_assistant_message ?? input.last_agent_message ?? "").trim();
  const template = loadPromptTemplate(ROOT_DIR, "stop-review-gate");
  const claudeResponseBlock = lastAssistantMessage
    ? ["Previous Antigravity response:", lastAssistantMessage].join("\n")
    : "";
  return interpolateTemplate(template, {
    ANTIGRAVITY_RESPONSE_BLOCK: claudeResponseBlock
  });
}
...
function main() {
  ...
  const cwd = input.cwd || process.env.ANTIGRAVITY_PROJECT_DIR || process.env.AGY_PROJECT_DIR || process.env.CLAUDE_PROJECT_DIR || process.cwd();
